...
- `zhttp-bench-server` auto-selects its permanent worker count from the benchmark reuse mode:
  `--reuse=1` uses `max(cpu_count, 16)` workers, while `--reuse=0` uses `cpu_count` workers.
