...
fn runWithSeed(
    ...
    if (cfg.test_filter) |f| {
        const filter_arg = try std.fmt.allocPrint(allocator, "--zhttp-match={s}", .{f});
        defer allocator.free(filter_arg);
        try argv.append(allocator, filter_arg);
    }

    if (!cfg.include_loopback_preflight) {
        const exclude_arg = try std.fmt.allocPrint(allocator, "--zhttp-skip={s}", .{"loopback listen preflight"});
        defer allocator.free(exclude_arg);
        try argv.append(allocator, exclude_arg);
    }
...
fn runSingleTestWithSeed(
    ...
    try argv.append(allocator, cfg.test_bin);
    try argv.append(allocator, "--zhttp-run-test");
...
fn printRepro(cfg: Config, seed: u32) void {
    ...
    if (cfg.test_filter) |f| {
        std.debug.print(" --zhttp-match \"{s}\"", .{f});
    }
    if (!cfg.include_loopback_preflight) {
        std.debug.print(" --zhttp-skip \"loopback listen preflight\"", .{});
    }
    std.debug.print("\n", .{});
}
...
fn printSingleTestRepro(seed: u32, test_name: []const u8) void {
    std.debug.print(
        "  zig build test -- --zhttp-run-test \"{s}\" --seed {d}\n",
        .{ test_name, seed },
    );
}
...
pub fn main(init: std.process.Init) !void {
    ...
    while (it.next()) |arg_z| {
        ...
        if (std.mem.eql(u8, arg, "--include-loopback-preflight")) {
            cfg.include_loopback_preflight = true;
            continue;
        }
